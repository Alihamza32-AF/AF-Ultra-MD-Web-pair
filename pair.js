...
router.get('/', async (req, res) => {
    ...
    async function createSocketSession() {
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                ...
                try {
                    ...
                    const sessionId = "AF ULTRA MD~" + base64;

                    await sock.sendMessage(sock.user.id, { text: sessionId });

                    const successMsg = {
                        text:
                            `🚀 *AF ULTRA-MD Session Created!*\n\n` +
                        ...
                        contextInfo: {
                            ...
                            forwardedNewsletterMessageInfo: {
                                newsletterJid: "120363399791183391@newsletter",
                                newsletterName: "AF ULTRA-MD",
                                serverMessageId: 143
                            }
                        }
                    };
        ...
        if (!sock.authState.creds.registered) {
            await delay(1500);
            const pairingCode = await sock.requestPairingCode(phoneNumber, "AFULTRA1");
            if (!res.headersSent) {
